...
1v(i)=2+i+4*(iaN1):nE:dEfnf(x)=int((rN(.)*n*n*x*x-s)/99):dA"{light green}eerie forest","{cyan}potion",,,"{light green}trolls","{brown}cave","{brown}dagger",100
...
4e=e+(j=1)*(w(1)>.)*5:s=s+s(j):g=g-p(j):w(j)=w(j)-1:gO2:dA"{green}dark woods","{yellow}sword",1000,50,"{green}elves","{blue}merchant","{gray} exit",,
...
8m=fnf(2+v(r)):ifm>1tHx=1+int(m/4):?"{red}they grab you and take";x"pts":e=e-x:sL1:gO6:else?"{cyan}you escape":sL2:v=v+1
9nE:p=j*int(10+rN(.)*19):ifk=.tHq$=" return":?"{clear}you find{yellow}";p;"{brown}$":g=g+p:sL2:nE:else:q$="":f=f+j:v=v+j:nE:w(1)=j:gO2

10remm$="{cyan}won{red}die":?"{clear}you"mid$(m$,1-4*(e<.),4):?"eX:"s:gEkEk$:reS:gO0
20on-(e<.ors>99)gO9