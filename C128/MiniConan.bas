...
2goS9:?"{orange}the inn{007}":fOi=1to4:?"{white}"i;j$(i):nE:inputj:on-(j<4)gO4:goS9:?j$(4):fOi=1to3:?"{white}"i"{dark gray}"w(i)m$(i)p(i):dA"{brown}armor{yellow}",300
...
6ife<1tH?"{down}end":sL3:goS9:gEkEk$:rU:elseifatH?"{cyan}you kill the "e$(r):f=f+1:p=int(rN(1)*r*n):?"{cyan}you get {yellow}${orange}"p:g=g+p:sL1:gO8
7m=fnf(v(r)):ifm>vaNb<1tHx=1+int(m/9/z):?"{red}they grab y"g$;x"p":e=e-x:vO9:vO.:sL1:gO5:else?"{cyan}escaped":sL1:v=v+1:b=b+(b>.)
8nE:p=j*int(9+rN(1)*9):ifk=.tHt=t+.1-(t>1)-(t>9):q$=r$:?"{clear}{light green}you find {yellow}${orange}"p"{light green}!{007}":g=g+p:sL2:nE:elseq$="":nE:w(1)=w(1)+j:gO2
9?"{clear}{red}S{pink}"e" {red}pw{white}"f"{cyan}sp{white}"v" {light green}bt{green}"b"{brown}ar{light gray}"z"{yellow}${orange}"g:?"{gray}score:{white}";v+f-2:?:reT:dA"{green}elves","{blue}merchant","{gray} exit",,"{green}werewolves",,,,